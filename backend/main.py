...
async def list_models():
    ...
    try:
        ...
        raise HTTPException(
            status_code=502, detail=f"Failed to fetch models: {e!s}"
        ) from e
...
async def refresh_models():
    ...
    try:
        ...
        raise HTTPException(
            status_code=502, detail=f"Failed to refresh models: {e!s}"
        ) from e
...
async def update_council_configuration(request: UpdateCouncilConfigRequest):
    ...
    save_council_config(
        deduped_council_models, request.chairman_model, request.web_search_enabled
    )
...
async def delete_conversations(confirm: bool = False):
    ...
    if not confirm:
        raise HTTPException(
            status_code=400, detail="Pass ?confirm=true to delete all conversations"
        )
...
async def delete_conversation(conversation_id: str):
    """Delete a single conversation from storage."""
    try:
