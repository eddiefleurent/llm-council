...
def _make_upload(filename: str, data: bytes, content_type: str = "text/plain") -> UploadFile:
    return UploadFile(filename=filename, file=io.BytesIO(data), headers=None, size=len(data))


@pytest.mark.asyncio
async def test_extract_text_file_success():
    upload = _make_upload("notes.txt", b"Hello council", "text/plain")
...
async def test_extract_json_file_success():
    upload = _make_upload("data.json", b'{"k": "v"}', "application/json")
    payload = await extract_attachment_payload(upload)
...
async def test_extract_pdf_file_success_with_mocked_reader(monkeypatch):
    ...
    upload = _make_upload("file.pdf", b"%PDF-1.4 fake", "application/pdf")
    payload = await extract_attachment_payload(upload)
...
async def test_extract_rejects_unsupported_extension():
    upload = _make_upload("archive.zip", b"PK\x03\x04", "application/zip")
...
async def test_extract_rejects_oversized_file():
    upload = _make_upload("big.txt", b"a" * (MAX_FILE_SIZE_BYTES + 1), "text/plain")
...
async def test_extract_rejects_invalid_json():
    upload = _make_upload("broken.json", b"{bad", "application/json")
